...
def main(args):
    ...
    _ensure_dir(OUT_BASE)  ### CHANGE: only ensure top-level

    import glob
    base = IN_ROOT / "image"
    folders = glob.glob(f"{base}/*/")   ### CHANGE: simple list of case folders
    ...
    for folder in tqdm(folders[:3], desc="Processing folders"):
        for i in range(1):
            ...
            # print("Using preset prompt: {}".format(present_prompts[i]))
            ...
            case_root = OUT_BASE
            case_image_root = case_root / "image" / case
            case_mask_root = case_root / "mask" / case 
            case_overlay_root = case_root / "overlay" / case

            _ensure_dir(case_image_root)
            _ensure_dir(case_mask_root)
            if args.save_overlay:
                _ensure_dir(case_overlay_root)
        ...
        for i in range(3):
            ...
            print(f"[{case}] Inference done. Saving results for prompt {i:03d}...\n")

            # prompt-level dirs: 000, 001, 002
            mask_prompt_dir = case_mask_root / f"{i:03d}"        ### CHANGE
            img_prompt_dir  = case_image_root       ### CHANGE
            _ensure_dir(mask_prompt_dir)
            _ensure_dir(img_prompt_dir)

            if args.save_overlay:
                overlay_prompt_dir = case_overlay_root / f"{i:03d}"  ### CHANGE
                _ensure_dir(overlay_prompt_dir)

            for pred_mask_vid in pred_masks:   ### CHANGE: we don't need inner i again
                ...
                for frame_idx in range(total_frames):
                    ## find original frame name 
                    frame_name = frame_name_list[frame_idx]
                    frame_name = frame_name.replace(".jpg", ".png")   ### CHANGE: save masks as PNG

                    pred_mask = pred_mask_vid[frame_idx].detach().cpu().numpy()
                    pred_mask = pred_mask > 0   # boolean

                    # ----- save mask (PNG) -----
                    mask_path = mask_prompt_dir / frame_name
                    binary_mask_uint8 = (pred_mask.astype(np.uint8) * 255)  ### CHANGE: correct dtype
                    cv2.imwrite(str(mask_path), binary_mask_uint8)

                    # ----- save original frame as JPG -----
                    if i == 0:   # save only once
                        img_path = img_prompt_dir / frame_name
                        # convert RGB → BGR for OpenCV
                        cv2.imwrite(str(img_path), image_list_np[frame_idx][:, :, ::-1])

                    # ----- save overlay -----
                    if args.save_overlay:
                        overlay_path = overlay_prompt_dir / frame_name
                        ...
                        save_img[pred_mask] = (
                            0.5 * save_img[pred_mask] +
                            0.5 * overlay_color
                        ).astype(np.uint8)

                        # RGB → BGR for saving
                        cv2.imwrite(str(overlay_path), save_img[:, :, ::-1])
